...
from muffin.eval.muffin_inference_logp import preference_collator_fn, concate_pad, concate_pad_three
...
from muffin.train.train_utils import SFT_collator_fn, IGNORE_INDEX, encode_multimodal_sample, encode_multimodal_preference_sample, encode_alttext_multimodal_sample
...
class AlttextDataset(Dataset):
    def __init__(self,
                 tokenizer: transformers.PreTrainedTokenizer,
                 multimodal_cfg: dict):
        super(AlttextDataset, self).__init__()
        file = ''
        with open(file, 'rb') as f:
            self.list_data_dict = json.load(f)  #[:1]
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        source: dict = self.list_data_dict[i]
        data_dict = encode_alttext_multimodal_sample(source, self.tokenizer, self.multimodal_cfg)
        return data_dict
...
class DPODataset(Dataset):
    def __init__(self,
        ...
        self.list_data_dict = RLHFVDataset(data_dir, ref_name, reference_model, tokenizer,
            multimodal_cfg['image_token_len'], multimodal_cfg['image_processor'], 
            multimodal_cfg['use_im_start_end'], tok_logits_path, use_tok, parquet_path,
            use_image_type)
        self.multimodal_cfg = multimodal_cfg
...
class DataCollatorForDPODataset(object):
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        batch = preference_collator_fn(instances, self.tokenizer.pad_token_id, mdpo=True)

        rej_instances, win_instances = list(zip(*instances))

        batch['uncond_ref_win_logp'] = torch.as_tensor([x['uncond_ref_win_logp'] for x in win_instances])
        batch['uncond_ref_rej_logp'] = torch.as_tensor([x['uncond_ref_rej_logp'] for x in rej_instances])
        ...
        batch['win_reference_vocab_ps'] = [x['win_reference_vocab_ps'] for x in win_instances]
        batch['rej_reference_vocab_ps'] = [x['rej_reference_vocab_ps'] for x in rej_instances]
        ...
        batch['concatenated_token_weight'] = concate_pad_three(win_token_weight, rej_token_weight, win_token_weight, 0)
...
def make_supervised_data_module(tokenizer: transformers.PreTrainedTokenizer,
                                data_args) -> Dict:
    train_dataset = AlttextDataset(tokenizer=tokenizer,
                                multimodal_cfg=dict(
...
def make_dpo_data_module(tokenizer, data_args, reference_model, training_args):
    train_dataset = DPODataset(tokenizer=tokenizer,
                               data_dir=data_args.data_dir,
                               ref_name=data_args.ref_name,
                               reference_model=reference_model,
                               tok_logits_path=training_args.tok_logits_path,
                               use_tok=training_args.use_tok,
                               parquet_path=training_args.parquet_path,
                               use_image_type=training_args.use_image_type,
                               multimodal_cfg=dict(
                                    is_multimodal=data_args.is_multimodal,
                                    image_token_len=data_args.image_token_len,
                                    image_folder=data_args.image_folder,
    ...
    data_collator = DataCollatorForDPODataset(tokenizer=tokenizer, mod_token_weight=data_args.dpo_token_weight)
    return dict(train_dataset=train_dataset,
                eval_dataset=None,
                data_collator=data_collator)
...
def init_model(model_args, data_args, training_args):
    ...
    if training_args.task == 'LM' or training_args.task == 'OCR':
        ...
        data_module = make_dpo_data_module(tokenizer, data_args=data_args, reference_model=ref_model, training_args=training_args)
        del ref_model
        ref_model = None
    return model.cuda(), data_module, tokenizer, ref_model
...
def train():
    ...
    model, data_module, tokenizer, ref_model = init_model(model_args, data_args, training_args)
